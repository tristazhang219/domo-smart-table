...
Modal.prototype = {
  ...
  constructor(data, {id, title, editable=false, actions}) {
    this.data = data;
    this.id = id;
    ...
    const createHandler = function(e){
      this.readForm();
      actions.createAction({content: this.formData}).then(data => {
    ...
    const deleteHandler = function(e){
      this.readForm();
      if (this.id) {
        actions.deleteAction(this.id, this.formData).then(data => {
          console.log("Delete complete", data);
          removeSpinner();
        });
      } else {
        console.error("Unable to delete document without given an id");
      }
      createSpinner();
    ...
    const saveHandler = function(e){
      this.readForm();
      if (this.id) {
        actions.saveAction(this.id, {content: this.formData}).then(data => {
          console.log("Save complete", data);
          removeSpinner();
        });
      } else {
        console.error("Document id must be provided to save a document");
      }
      createSpinner();
    ...
    const btnsDef = editable ? 
                      ( !!actions.createAction ? 
                        [{name: "Cancel", class: 'cancel', action: this.close.bind(this)},
                          {name: "Create", class: 'create', action: createHandler}]
                        : [{name: "Cancel", class: 'cancel', action: this.close.bind(this)},
                          {name: "Delete", class: 'delete', action: deleteHandler },
                          {name: "Save", class: 'save', action: saveHandler }] )
                    : [{name: "OK", class: 'cancel', action: this.close.bind(this)}];
